...
const ResetPass = () => {
  ...
  const onSubmitOtp = async (e)=>{
     ...
     setOtp(otpValue)
     try{
      const {data} = await axios.post(backend_url + "/api/auth/verify-reset-otp", {email, otp: otpValue})
      if(data.success){
      ...
      <form onSubmit={onSubmitEmail}
       className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
        ...
        rounded-full bg-[#333A5C]'>
          ...
          className='bg-tranparent outline-none text-white'
          value={email} onChange={e => setEmail(e.target.value)} required/>
        </div>
        <button className='w-full py-3 bg-gradient-to-r
              from-indigo-500 to-indigo-600 text-white rounded-full cursor-pointer'>Submit</button>
      </form>
}
     ...
     {!isOtpSubmited && isEmailSent && 
          ...
          <form  onSubmit={onSubmitOtp}
            className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
              ...
              <p className='text-center mb-6 text-indigo-300'>Enter the 6-digit code sent ro your email</p>
